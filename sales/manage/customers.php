...
//if (isset($_GET['debtor_no'])) 
//{
//	$_POST['ipd_customer_id'] = $_GET['ipd_debtor_no'];
//}

$ipd_selected_id = get_post('ipd_customer_id','');
...
function can_process()
{
	...
	if ($_POST['sex'] != 'M'&&$_POST['sex'] != 'm'&&$_POST['sex'] != 'F'&&$_POST['sex'] != 'f') 
	{
		display_error(_("The customer Sex cannot be empty. Write M/m for male customer and F/f for female customer"));
		set_focus('sex');
		return false;
	}
	if (!check_num('age', 0))
	{
		display_error(_("The customer age must be numeric and not less than zero."));
		set_focus('age');
		return false;		
	} 
...
function handle_submit(&$selected_id)
{
	...
	{ 	//it is a new customer
                $_POST['address']="Age: ".$_POST['age']." Y"."\nSex: ".(($_POST['sex']=='M'||$_POST['sex']=='m')? 'Male':'Female');
		begin_transaction();
		...
		meta_forward( $path_to_root.'/sales/sales_order_entry.php?NewInvoice=0&branch_id='.$selected_branch.'&doc_ref='.$_POST['doc_ref'].'&age='.$_POST['age'].'&sex='.$_POST['sex']);

		display_notification(_("A new customer has been added.$selected_branch"));
...
if (isset($_POST['submit'])) 
{
	...
	//header('Location: /sales/sales_order_entry.php?NewInvoice=0&branch_id=$selected_branch');
...
function customer_settings($selected_id) 
{
	...
	if (!$selected_id) 
	{
	 	if (list_updated('customer_id') || !isset($_POST['CustName'])) {
			$_POST['CustName'] = $_POST['cust_ref'] =  $_POST['tax_id']  = $ipd_selected_id;
			$_POST['address'] ='N/A';
			...
			$_POST['pymt_discount'] = percent_format(0);
			$_POST['credit_limit']	= price_format($SysPrefs->default_credit_limit());
		}
                if($ipd_selected_id && list_updated('ipd_customer_id')){
                            $myrow2 = ipd_get_customer($ipd_selected_id);
                            
                            $_POST['CustName'] = $myrow2["name"];
                            $_POST['cust_ref'] = $myrow2["debtor_ref"];
                            $_POST['address']  = $myrow2["address"];
                            $_POST['tax_id']  = $myrow2["tax_id"];
                            $_POST['dimension_id']  = $myrow2["dimension_id"];
                            $_POST['dimension2_id']  = $myrow2["dimension2_id"];
                            $_POST['sales_type'] = $myrow2["sales_type"];
                            $_POST['curr_code']  = $myrow2["curr_code"];
                            $_POST['credit_status']  = $myrow2["credit_status"];
                            $_POST['payment_terms']  = $myrow2["payment_terms"];
                            $_POST['discount']  = percent_format($myrow2["discount"] * 100);
                            $_POST['pymt_discount']  = percent_format($myrow2["pymt_discount"] * 100);
                            $_POST['credit_limit']	= price_format($myrow2["credit_limit"]);
                            $_POST['notes']  = $myrow2["notes"];
                            $_POST['inactive'] = $myrow2["inactive"];
                        }
	}
	else 
	{
	...
	text_row(_("Age"), 'age', null, 10, 2);
	text_row(_("Sex (M/F)"), 'sex', null, 1, 1);
	text_row(_("Doctor Reference:"), 'doc_ref', null, 40, 150);
...
//		email_row(_("E-mail:"), 'email', null, 35, 55);
		...
		switch($_SESSION["wa_current_user"]->user){
               
                case ('2'):$s_man=2;break;
                case ('3'):$s_man=3;break;
                case ('13'):$s_man=4;break;
                case ('15'):$s_man=5;break;
                case ('17'):$s_man=6;break;
                    
                default : $s_man=1;
                }
		sales_persons_list_row( _("Sales Person:"), 'salesman', $s_man);
	}
...
if (ipd_db_has_customers()) 
{
	start_table(TABLESTYLE_NOBORDER);
	start_row();
	ipd_customer_list_cells(_("Select a IPD: "), 'ipd_customer_id', null,
		_('New IPD'), true, check_value('ipd_show_inactive'));
	check_cells(_("Show inactive:"), 'ipd_show_inactive', null, true);
	end_row();
	end_table();

	if (get_post('_ipd_show_inactive_update')) {
		$Ajax->activate('ipd_customer_id');
		set_focus('ipd_customer_id');
	}
} 
else 
{
	hidden('ipd_customer_id');
}
