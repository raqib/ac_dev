...
include_once($path_to_root . "/includes/ui/ui_controls.php");
...
if (isset($_GET['AddedID'])) {
	...
	$invoice = $_GET['AddedDI'];
        $amnt=user_numeric($_GET['paid']);
        $dis=user_numeric($_GET['discount']);

	//display_notification_centered(sprintf(_("Invoice # %d has been entered."), $invoice));
//        if(!isset($_GET['theinvoice']))
        fa_popup($path_to_root."/sales/customer_payments_1.php?SInvoice=".$invoice."&customer_id=".$_GET['customer_id']."&AddPaymentItem=1&amount=".$amnt."&discount=".$dis."&bug=1");
    
//        else
//        $invoice=$_GET['theinvoice'];
//        http://localhost/fadev/sales/customer_payments.php?SInvoice=17&customer_id=2&AddPaymentItem=1&amount=2&discount=1
        //meta_forward($path_to_root.'/reporting/prn_redirect.php?PARAM_0=20-10&PARAM_1=20-10&PARAM_2=&PARAM_3=0&PARAM_4=&PARAM_5=&PARAM_6=0&REP_ID=107');
...
function cust_can_process()
{
	...
	if (strlen($_POST['ipd']) != 4&&strlen($_POST['ipd'])!=0) 
	{
		display_error(_("Type 'c' for cabin and 'w' for word along with room no.e"));
		set_focus('ipd');
		return false;
	} if ($_POST['free']!=''&& ($_POST['free']!='f'||$_POST['free']!='F')) 
	{
		display_error(_("Write 'F' at free field if the bill is free or leave blank "));
		set_focus('free');
		return false;
	} 
...
if (isset($_POST['ProcessOrder']) && can_process()) {
	...
	else
	{
		...
		if ($modified) {
			...
			meta_forward($_SERVER['PHP_SELF'], "AddedDI=".$trans_no."&Type=".$so_type."&customer_id=".$customer_id."&discount=".$discount."&paid=".$paid);
		} else {
...
if ($customer_error == "") {
	...
	echo "<tr><td>";
		...
		if(isset($_GET['doc_ref']))
                    $_SESSION['Items']->cust_ref=$_GET['doc_ref'];
                else if(isset($_POST['doc_ref']))
                    $_SESSION['Items']->cust_ref=$_POST['doc_ref'];
                if(isset($_GET['ipd']))
                    $_SESSION['Items']->ipd=$_GET['ipd'];
                else if(isset($_POST['ipd']))
                    $_SESSION['Items']->ipd=$_POST['ipd'];
                if(isset($_GET['free']))
                    $_SESSION['Items']->free=$_GET['free'];
                else if(isset($_POST['free']))
                    $_SESSION['Items']->free=$_POST['free'];
		if(isset($_GET['age']))
		$_SESSION['Items']->delivery_address="Age: ".$_GET['age']." Y"."\tSex: ".(($_GET['sex']=='M'||$_GET['sex']=='m')? 'Male':'Female');
