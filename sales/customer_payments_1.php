...
if(isset($_GET['AddPaymentItem']))
$_POST['AddPaymentItem']=$_GET['AddPaymentItem'];
if(isset($_GET['BranchID']))
$_POST['BranchID']=$_GET['BranchID'];

if(isset($_GET['discount']))
$_POST['discount']=$_GET['discount'];
if(isset($_GET['amount']))
$_POST['amount']=$_GET['amount'];
$temp_inv=$_GET['SInvoice'];
		...
		if($inv) {
			...
			foreach($_SESSION['alloc']->allocs as $line => $trans) {
				if ($trans->type == ST_SALESINVOICE && $trans->type_no == $_GET['SInvoice']) {
					$_POST['amount'] =
						$_SESSION['alloc']->amount = (isset($_GET['amount'])? $_GET['amount']:price_format($_SESSION['alloc']->allocs[$line]->amount));
					$_SESSION['alloc']->allocs[$line]->current_allocated =
                                                (isset($_GET['amount'])? $_GET['amount']:$_SESSION['alloc']->allocs[$line]->amount);
					break;
				}
			}
			unset($inv);
...
if (isset($_GET['AddedID'])) {
	$payment_no = $_GET['AddedID'];
        $bill=$_GET['theinvoice'];
        
        meta_forward($path_to_root.'/reporting/prn_redirect.php?PARAM_0='.$bill.'-10&PARAM_1='.$bill.'-10&PARAM_2=&PARAM_3=0&PARAM_4=&PARAM_5=&PARAM_6=0&REP_ID=107');
        //meta_forward($path_to_root."/sales/sales_order_entry.php?AddedDI=1&type=0&theinvoice=".$_GET['theinvoice'].'&dfd');
...
function can_process()
{
	...
	if (!isset($_POST['DateBanked']) || !is_date($_POST['DateBanked'])) {
		...
		display_error(_($_POST['DateBanked']."The entered date is not in fiscal year."));
		set_focus('DateBanked');
		return false;
	}
...
if (get_post('AddPaymentItem') && can_process()) {
	...
	meta_forward($_SERVER['PHP_SELF'], $new_pmt ? "AddedID=$payment_no" : "UpdatedID=$payment_no");
        ...
        meta_forward($_SERVER['PHP_SELF'], ($new_pmt ? "AddedID=$payment_no" : "UpdatedID=$payment_no").'&theinvoice='.$temp_inv.'&df');
}
?>
